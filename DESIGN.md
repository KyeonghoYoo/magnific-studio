...
| **Remotion** | 커스텀(4인+ 유료 — 코드 미사용) | 정규화 워드타이밍 `Caption` 타입(실제 `{text,startMs,endMs,timestampMs,confidence}`) + 카라오케 페이징 **개념** | 개념만 참고해 **자체 스키마 `{word,start_ms,end_ms}`**로 정의, libass `\k` ASS 렌더 구현(라이선스 회피) |
...
- **숏 생성 전략(v0.7.0, 로드맵 4)**: 숏마다 `generation_strategy`(generative·stock·hybrid). 캐릭터·브랜드 식별자가 없는 설정/B롤/인서트는 `stock`(Magnific 스톡 `stock_search`→`stock_to_creation`)으로, 캐릭터 등장·감정 비트·제품은 `generative`, 스톡 환경+생성 전경은 `hybrid`. 비히어로 숏의 생성 과금을 없애 예산을 히어로에 집중 — 비용 게이트 강화. 비용 견적은 stock 숏 제외 산정.
...
- **정본 `edit_plan.json`(edit-as-data, 버전드·멱등).** 에이전트가 컷/트림/전환/자막/음악/납품본을 선언적 EDL로 기술하고, ffmpeg 실행기가 렌더한다. `schema_version`·`pipeline_version` + `render` 블록(모든 인코딩 파라미터 명시)으로 **동일 계획+소스 → 프레임 결정적 재렌더**(v0.6.0). `source.clips` 지문으로 소스 변경을 탐지하고, `variants[]` 오버라이드로 **생성 재소비 0의 A/B 다변형**을 뽑는다. 렌더 실패·재편집이 계획 데이터 수정으로 환원(ShortGPT 패턴). 사용자 확정 후에만 최종 렌더.
...
- **자막 = libass 우선.** 한글은 Pretendard/Noto Sans CJK KR, ASS `Alignment 2`+`MarginV≈360`(릴스 세이프존). 워드 카라오케는 Remotion Caption 개념을 참고한 자체 워드타이밍 스키마(`{word,start_ms,end_ms}`) → WhisperX 워드타이밍 → ASS `\k` 렌더. 한글은 `.ass`/`textfile`로 넘겨 셸 이스케이프 회피.
- **음악 싱크 = 측정.** librosa `beat_track`/`onset`/`rms`로 컷을 비트에 스냅하고, RMS 빌드 지점을 서사 전환점(예: 라이딩 시작)에 정렬(실전: 오프셋 5s).
- **오디오 스택(v0.8.0, 로드맵 5).** 음악 + 내레이션 VO(`audio_tts`+`audio_voices_list`, 견적 게이트) + VO→WhisperX 워드타이밍 카라오케 자막(`captions[].word_timings`) + VO 사이드체인 덕킹. 완성 릴스의 테이블 스테이크스를 정식 후반 계약(`audio.voiceover`/`ducking`)으로 편입.
...
- **채택 로드맵(Adopt-now 상위 5)**: (1) **✅ v0.4.0** — 캐릭터 일관성 계약 + 참조 뱅크(하류가 id로 인용 강제, 프로버넌스), (2) **✅ v0.5.0** — 스테이지별 자동 자기검수 QA(`quality-reviewer`) → 승인 게이트에 증거 부착(축별 `review`), (3) **✅ v0.6.0** — `edit_plan` EDL 버전드 스키마 + 재렌더 멱등성(schema_version·render 블록·source.clips 지문·variants A/B), (4) **✅ v0.7.0** — 콘티 계약에 `generative|stock|hybrid` 숏 전략(Freepik/Magnific 스톡으로 비히어로 비용 절감), (5) **✅ v0.8.0** — 표준 후반 오디오 스택(TTS 내레이션 + VO 워드타이밍 카라오케 자막 + BGM 덕킹). **→ Adopt-now 로드맵 5/5 완주(v0.4.0→v0.8.0).**
