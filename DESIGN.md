...
- **채택 로드맵(Adopt-now 상위 5)**: (1) **✅ v0.4.0** — 캐릭터 일관성 계약 + 참조 뱅크(하류가 id로 인용 강제, 프로버넌스), (2) **✅ v0.5.0** — 스테이지별 자동 자기검수 QA(`quality-reviewer`) → 승인 게이트에 증거 부착(축별 `review`), (3) **✅ v0.6.0** — `edit_plan` EDL 버전드 스키마 + 재렌더 멱등성(schema_version·render 블록·source.clips 지문·variants A/B), (4) 콘티 계약에 `generative|stock|hybrid` 숏 전략(Freepik 스톡으로 비히어로 비용 절감), (5) 표준 후반 오디오 스택(TTS+자막 타이밍+BGM 덕킹).
