...
├── characters.json           # 2단계 정본 (Library identifier 포함)
...
- `characters.json`에 각 캐릭터의 library identifier + 3뷰 creation identifier + 특징 기술을 기록.
...
4. **키프레임 품질 루프** (ViMax 미배선 기능의 완성): 키프레임을 N=2~3장 variation 생성 → 캐릭터 일관성(이목구비/체형/헤어), 공간 일관성(배치), 묘사 정확성 3축으로 심사 후 최선 채택.
