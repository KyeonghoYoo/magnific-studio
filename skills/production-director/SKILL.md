...
먼저 `magnific-studio-core`와 `spaces-engineer` 스킬을 읽었는지 확인한다.
...
1. 참조 선택 휴리스틱(spaces-engineer)에 따라 캐릭터 Library 자산 + 인접 프레임을 참조로 연결.
2. **variation 2~3장 생성** 후 3축 심사로 최선을 채택:
   - **캐릭터 일관성** — 이목구비·체형·헤어가 characters.json 시트 및 참조와 일치
   - **공간 일관성** — 인물 배치(좌/우), 배경 구도가 인접 숏과 모순 없음
   - **묘사 정확성** — first_frame_desc의 내용이 실제로 구현됨
3. 채택 프레임의 creation identifier를 manifest에 기록. 탈락 사유는 decision_log에.
4. 같은 camera.position_id의 첫 채택 프레임은 그 포지션의 **앵커 프레임**으로 지정 — 이후 해당 포지션 숏의 참조 풀에 항상 포함한다.
...
클립은 재생 없이도 검수 가능하다: `creations_get`이 반환하는 **preview-grid**(50여 프레임 시트)와 start/end frame으로 프레임 단위 심사를 한다. 심사 축: ① 모션 자연스러움(워핑·텔레포트·배경 유동), ② 캐릭터/차량 정체성 유지, ③ 카메라 지시 준수(방향·무브먼트), ④ 시작/끝 프레임이 키프레임과 일치하는가. FilmAgent의 비평-수정 패턴처럼 **불합격 클립은 원인을 분류**(키프레임 결함 / 프롬프트 모호 / 모델 한계)하고 해당 층에서 고친다 — 키프레임 결함을 프롬프트나 모델 승급으로 때우려 하면 반복 실패한다. 원인과 조치는 decision_log에 기록.
