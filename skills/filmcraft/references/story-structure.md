...
| polarity progression | 극성 진행 | chaining scenes | escalating AMPLITUDE, not just alternating sign | scene(n+1) flips kind AND |Δ| grows toward the midpoint | AI clips are planned independently — carry the prior scene's end-polarity into the next scene's brief ("continuing from a false-victory high…") |
