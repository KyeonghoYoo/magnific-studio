...
| Shot Matching | 샷 매칭 | any scene with >1 shot, after correction | invisible continuity; breaks immersion if skipped | N/A — MANUAL ffmpeg pass (no renderer path yet — record ops in edit_plan notes for reproducibility): trim every shot's `colorbalance`/`eq` to a **hero shot** (the best-exemplar frame), never to a synthetic averaged histogram | independently generated clips drift in WB/saturation from similar prompts — matching is mandatory, not optional |
| Color Grading (Creative Look) | 컬러 그레이딩 | after shots are matched — a scene/film-level pass | shapes emotional register independent of literal frame content | Prompt: name the look ("bleach-bypass war grade"), never raw parameters — ffmpeg: `lut3d` show LUT (implemented, uniform) — per-scene `colorbalance`/`curves` trim is a DECLARED MANUAL pass recorded in edit_plan notes (roadmap) | zero hex/%/numeric splits in prompt text — hard lint |
...
| LUT (1D/3D .cube) | 룩업테이블(LUT) | applying one consistent look across many shots in one operation | bakes a look into one deterministic op | N/A — ffmpeg: `lut3d=file=look.cube` (or haldclut) | the renderer applies `edit_plan.color.show_lut` as ONE uniform pass — keep it conservative so it does not flatten a designed multi-scene color script; per-scene variance is enforced at GENERATION time until the manual trim pass lands (see spine); LUT = the file, "look" = the creative target, "grade" = the act |
...
6. **No persistent state is why any of this exists.** Hero-shot matching, Color Script re-injection, location-signature restatement, and the conservative uniform show LUT (per-scene palette enforced at generation time) all exist because prompted color drifts shot to shot even from identical prompts.
...
- **Drift workarounds, in priority order:** Color Script re-injected per beat into every shot's prompt · same reference/style image reused across a scene's generations · conservative uniform show LUT (masks residual drift, doesn't fix it; per-scene trim = declared manual pass) · locked lighting-descriptor vocabulary shared with `lighting.md`.
