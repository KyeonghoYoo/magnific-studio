...
**D7. J/L-cuts are authored by CUE PLACEMENT; there are no signed-offset fields.** A pre-lap (J-cut) is expressed by setting the incoming element's in-point earlier than the picture-cut timestamp (`music_cues[].in` / `sfx[].at_sec`; VO has NO in-point field — author a VO pre-lap by padding leading silence into the TTS render, roadmap: `voiceover.start_sec`); a post-lap (L-cut) by extending the outgoing element's out-point past the cut (`music_cues[].out`; VO has NO out-point field — a VO trail is trailing silence in the TTS render). J-cut/L-cut exist only as glossed aliases in prose/UI (§5) — they are the most-reversed pair in practice, and explicit cue timestamps can't be misread the way letter-names can. Fields like `audio_in_offset_ms`/`audio_out_offset_ms` do NOT exist in edit_plan — never cite them.
...
- **Captions:** budgeted in CHARACTERS — a count distinct from VO words/syllables above; no house per-duration character table defined yet (open item, do not improvise numbers here).
