...
**VO 배치는 실효 타임라인 기준(하드).** 전환(dissolve/dip/blur) 겹침은 총 길이를 당긴다(offset_k 산식) — `start_sec + VO길이 ≤ 전환 반영 후 실효 길이 − 0.2s`를 지켜야 말미가 안 잘린다(파일럿 1 실증: "라이덜리"의 "덜리" 잘림). 렌더러가 오버런을 경고하지만 배치 단계에서 먼저 산술하라.
...
### Step 2.5 — drawtext 부재 환경의 캡션 (실증 레시피)

일부 ffmpeg 빌드(brew 기본)는 drawtext/subtitles 필터가 없다. 이때 `caption_style.engine: "ass"`로 렌더러 자막을 스킵하고, **Pillow로 자막 PNG(1080×1920 투명, 세이프존 내)를 렌더한 뒤 2차 패스로 번인**한다 — 오디오는 copy라 라우드니스 불변:
```
ffmpeg -i master.mp4 -i caption.png -filter_complex \
  "[0:v][1:v]overlay=0:0:enable='between(t,START,END)'[v]" \
  -map "[v]" -map 0:a -c:v libx264 -crf 18 -c:a copy out.mp4
```
로고 등 실물 자산 합성도 동일 패턴(흰 배경 로고는 외곽 flood-fill 투명화 후 오버레이 — 파일럿 1 실증). 스피드 램프 구간의 디제틱 오디오는 기본 페이드아웃(음악 리프트가 지배)이고, 유지가 필요하면 `atempo`(피치 보존)로 별도 스템을 만들어 sfx로 배선한다.
...
| `blur_dissolve` | 감정 페이오프/브랜드 카드 소프트 진입(디포커스) — dip_to_white가 과할 때 | 600–1200 |
